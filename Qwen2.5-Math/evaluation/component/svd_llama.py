...
class SVD_LlamaMLP(nn.Module):
    ...
    def forward(self, x):
        x = x.to(self.up_v_proj.weight.dtype)
...
class SVD_LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, ratio=1):
        ...
        self.q_u_proj = nn.Linear(low_rank, self.proj_dim, bias=False, device=device)
        self.q_v_proj = nn.Linear(self.hidden_size, low_rank, bias=False, device=device)
        
        self.k_u_proj = nn.Linear(low_rank, self.proj_dim, bias=False, device=device)
        self.k_v_proj = nn.Linear(self.hidden_size, low_rank, bias=False, device=device)

        self.v_u_proj = nn.Linear(low_rank, self.proj_dim, bias=False, device=device)
        self.v_v_proj = nn.Linear(self.hidden_size, low_rank, bias=False, device=device)

        self.o_u_proj = nn.Linear(low_rank, self.hidden_size, bias=False, device=device)
        self.o_v_proj = nn.Linear(self.proj_dim, low_rank, bias=False, device=device)

        self.rotary_emb = LlamaRotaryEmbedding(self.head_dim, max_position_embeddings=self.max_position_embeddings).to(device)

    def _shape(self, tensor: torch.Tensor, seq_len: int, bsz: int):
        return tensor.view(bsz, seq_len, self.num_heads, self.head_dim).transpose(1, 2).contiguous()
    
    def start(self, config: LlamaConfig, ratio=1):
        super().__init__()
        print("================== Start!!! =========================")
        self.config = config
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        hidden_states = hidden_states.to(dtype=self.q_v_proj.weight.dtype)
